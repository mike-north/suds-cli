...
export class TextareaModel {
  ...
  lineUp(n = 1): TextareaModel {
    const target = clamp(this.pos.line - Math.max(1, n), 0, this.lines.length - 1);
  ...
  insertRunes(text: string): TextareaModel {
    ...
    const newColumn =
      parts.length === 1 ? this.pos.column + runeCount(parts[0]!) : runeCount(last(parts)!);
  ...
  #deleteLeftOnce(): TextareaModel {
    if (this.pos.column > 0) {
      ...
      return this.#withValue(
        replaceLine(this.lines, this.pos.line, merged),
        { line: this.pos.line, column: this.pos.column - 1 },
      );
    }
    if (this.pos.line === 0) {
      return this;
  ...
  deleteLine(): TextareaModel {
    ...
    const nextColumn = clamp(this.pos.column, 0, runeCount(nextLines[nextLine] ?? ""));
  ...
  duplicateLine(): TextareaModel {
    ...
    return this.#withValue(nextLines, { line: this.pos.line + 1, column: this.pos.column });
  }

  /** Scroll up by n lines (no cursor move). */
  scrollUp(n = 1): TextareaModel {
    const nextScroll = clamp(this.scrollTop - Math.max(1, n), 0, this.maxScroll());
  ...
  update(msg: Msg): [TextareaModel, Cmd<Msg>] {
    ...
    if (msg instanceof KeyMsg && (msg.key.type === KeyType.Left || msg.key.type === KeyType.Right || msg.key.type === KeyType.Home || msg.key.type === KeyType.End)) {
      const refreshed = model.cursor.withChar(model.cursor.isBlinkHidden() ? "▌" : model.cursor.char);
