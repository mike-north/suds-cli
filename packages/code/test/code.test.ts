...
describe("CodeModel", () => {
  ...
  it("creates a model with active=true", () => {
    const model = CodeModel.new({ active: true });
    expect(model.active).toBe(true);
  });

  it("creates a model with custom theme", () => {
    const model = CodeModel.new({ syntaxTheme: "monokai" });
