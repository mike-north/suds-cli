...
export class TextInputModel {
  ...
  #placeholderView(availableWidth: number): string {
    ...
    const budget = Number.isFinite(availableWidth)
      ...
      : undefined
    const visibleRest = budget === undefined ? rest : clampWidth(rest, budget)
  ...
  #valueView(availableWidth: number): string {
    ...
    const after =
      cursorOffset < slice.length ? slice.slice(cursorOffset + 1).join('') : ''

    const cursorChar =
      cursorOffset < slice.length ? (slice[cursorOffset] ?? ' ') : ' '
    ...
    const usedWidth =
      stringWidth(before) + stringWidth(cursorChar || ' ') + stringWidth(after)
    const padding =
      Number.isFinite(availableWidth) && availableWidth > 0
        ? Math.max(0, availableWidth - usedWidth)
        : 0
