...
class DemoModel implements Model<Msg, DemoModel> {
  ...
  constructor(editor?: TextareaModel, initCmd?: Cmd<Msg>) {
    ...
    const base = TextareaModel.new({
      ...
      placeholderStyle,
      validate: (v) =>
        v.trim().length < 5 ? new Error('Need at least 5 chars') : null,
    })
  ...
  view(): string {
    ...
    lines.push(
      infoStyle.render(
        'Controls: type • enter for newline • ctrl+v paste • q/ctrl+c quit',
      ),
    )
...
/**
 * Run the textarea demo with the given platform adapter.
